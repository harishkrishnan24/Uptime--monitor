...
// Lookup all the checks, send data to validator
...
workers.validateCheckData = function (originalCheckData) {
    ...
    // Set the keys that may not be set if the worker has never seen this check before
...
// Perform the check, send the originalCheckData and the outcome of the check process and pass to the next process
workers.performCheck = function (originalCheckData) {
    ...
    var path = parsedUrl.path // Using path and pathname because we need query string
...
workers.alertUserToStatusChange = function (newCheckData) {
    ...
    helpers.sendTwilioSms(newCheckData.userPhone, msg, function (err) {
        if (!err) {
            ...
            console.log('Error: Could not send sms alert to a user who has state change in their check');
        }
    });
};
...
workers.init = function () {
    ...
    // Call the loop so the checks will execte later on
    workers.loop();
