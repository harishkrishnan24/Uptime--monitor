...
handlers._users.post = function (data, callback) {
    ...
    var firstName = typeof(data.payload.firstName) == 'string' && data
        .payload
        .firstName
        .trim()
        .length > 0
        ? data
            ...
            .trim()
        : false;
    var lastName = typeof(data.payload.lastName) == 'string' && data
        .payload
        .lastName
        .trim()
        .length > 0
        ? data
            ...
            .trim()
        : false;
    var phone = typeof(data.payload.phone) == 'string' && data
        .payload
        .phone
        .trim()
        .length == 10
        ? data
            ...
            .trim()
        : false;
    var password = typeof(data.payload.password) == 'string' && data
        .payload
        .password
        .trim()
        .length > 0
            ...
            handlers
                ...
                .verifyToken(token, phone, function (tokenIsValid) {
                    if (tokenIsValid) {
                        // lookup for user
                        _data
                            .read('users', phone, function (err, userData) {
                                if (!err && userData) {
                                    // Update fields
                                    if (firstName) {
                                    ...
                                    // Store the updates
                                    _data
                                        .update('users', phone, userData, function (err) {
                                            if (!err) {
                                                callback(200);
                                            } else {
                ...
                if (tokenIsValid) {
                    ...
                    _data
                        .read('users', phone, function (err, data) {
                            if (!err && data) {
                                _data
                                    .delete('users', phone, function (err) {
                                        if (!err) {
                                            ...
                                            if (checksToDelete > 0) {
                                                ...
                                                userChecks.forEach(function (checkId) {
                                                    // Delete the check
                                                    _data
                                                        .delete('checks', checkId, function (err) {
                                                            if (err) {
                                                                deletionErrors = true;
                                                            }
                                                            checksDeleted++;
...
handlers._tokens.get = function (data, callback) {
    ...
    var id = typeof(data.queryStringObject.id) == 'string' && data
        .queryStringObject
        .id
        .trim()
        .length == 20
    ...
    if (id) {
        // lookup for user
        _data
            .read('tokens', id, function (err, tokenData) {
                if (!err && tokenData) {
...
handlers._tokens.put = function (data, callback) {
    var id = typeof(data.payload.id) == 'string' && data
        .payload
        .id
        .trim()
        .length == 20
...
handlers._tokens.delete = function (data, callback) {
    ...
    if (id) {
        // lookup for user
        _data
            .read('tokens', id, function (err, data) {
                if (!err && data) {
                    _data
                        .delete('tokens', id, function (err, data) {
                            if (!err) {
                                callback(200);
                            } else {
...
handlers._checks.post = function (data, callback) {
    ...
    var protocol = typeof(data.payload.protocol) == 'string' && ['https', 'http'].indexOf(data.payload.protocol) > -1
        ...
        : false;
    var url = typeof(data.payload.url) == 'string' && data
        .payload
        .url
        .trim()
        .length > 0
        ? data
            ...
            .trim()
        : false;
    var method = typeof(data.payload.method) == 'string' && ['put', 'get', 'post', 'delete'].indexOf(data.payload.method) > -1
        ? data.payload.method
        : false;
...
handlers._checks.get = function (data, callback) {
    ...
    if (id) {
        // Lookup the check
        _data
            .read('checks', id, function (err, checkData) {
                if (!err && checkData) {
                    //  Get the token from the headers
                    var token = typeof(data.headers.token) == 'string'
                        ? data.headers.token
                    ...
                    handlers
                        ._tokens
                        .verifyToken(token, checkData.userPhone, function (tokenIsValid) {
                            if (tokenIsValid) {
...
handlers._checks.put = function (data, callback) {
    // Required
    var id = typeof(data.payload.id) == 'string' && data.payload.id.length == 20
        ? data.payload.id
        : false;
                        ...
                        handlers
                            ...
                            .verifyToken(token, checkData.userPhone, function (tokenIsValid) {
                                if (tokenIsValid) {
                                    ...
                                    // Store
                                    _data
                                        .update('checks', id, checkData, function (err) {
                                            if (!err) {
                                                callback(200);
                                            } else {
                    ...
                    handlers
                        ...
                        .verifyToken(token, checkData.userPhone, function (tokenIsValid) {
                            if (tokenIsValid) {
                                ...
                                _data
                                    .delete('checks', id, function (err) {
                                        if (!err) {
                                            _data
                                                .read('users', checkData.userPhone, function (err, userData) {
                                                    if (!err && userData) {
                                                        var userChecks = typeof(userData.checks) == 'object' && userData.checks instanceof Array
                                                            ? userData.checks
                                                            : [];
